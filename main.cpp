...
public:
    //构造函数
    Identity(const char Name[32], int Number );
    //复制构造函数
...
Identity::Identity(const char Name[32]="无名氏", int Number = 123456) :number_{ Number }
{
...
Student MakeStudent(int number, const char* name, std::string school)    
{
...
{
    //调用Identity的默认构造函数
    Identity person1,person2("李华",654321);
    //调用Identity的复制构造函数
    ...
    //调用MakeStudent函数
    Student pupil_3 = MakeStudent(666666,"林俊杰","清华大学");
    pupil_3.show();
